...
class WeChat extends Model
{
    ...
    public function createImg1($tel='',$type=1,$poster_path='',$qrcode_path="./qrcode_temp/"){
        ...
        if(!$qrcode){
            ...
            //header('Content-type: image/jpg');
            //$qr_code = $this->getImg($url);
             // $url2 = Yii::$app->urlManager->createAbsoluteUrl(['index/register','invitation'=>$this->user_info->invitation]);
    ...
    public function createImg2($tel='',$type=1,$poster_path='',$qrcode_path="./qrcode_temp2/"){
        ...
        if(!$qrcode){
           ...
           // return 33;
            //写的文字用到的字体。字体最好用系统有得，否则会包charmap的错，这是黑体
            //$font = "simhei.ttf";   //写的文字用到的字体。字体最好用系统有得，否则会包charmap的错，这是黑体
            imagettftext($dst_qr,20,10,260,650,$rqys,$font,$nickname);
            imagettftext($dst_qr,18,0,260,1080,$rqys,$font,'长按二维码加入');
            ...
            //imagedestroy($head_img);
            ob_start();//启用输出缓存，暂时将要输出的内容缓存起来
            return imagejpeg($dst_qr,$qrcode_path.$tel.$str.".jpg",90);//输出
            imagedestroy($dst_qr);
            ob_end_clean();//清空缓存
            ob_end_clean();//清空缓存
            ob_end_clean();//清空缓存
