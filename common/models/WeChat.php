...
class WeChat extends Model
{
    ...
    public function init()
    {
        ...
        $this->_appid = Yii::$app->params['wx']['app_id'];
        $this->_appsecret = Yii::$app->params['wx']['app_secret'];
        $this->_token = Yii::$app->params['wx']['token'];
        $this->_appkey = '';
    }
    ...
    public function responseMsg(){
        ...
        file_put_contents('responseMsg.txt',22);
        if(!empty($xml_str)){
        ...
        if($request_xml->EventKey=='QRCODE') {
            ...
            if($user_info) {
                if($user_info->grade == 0) {
                    ...
                    $content = '您还未加入会员，<a href="'.$url.'">【请点击这里购买】</a>';
                    $this->_msgText($request_xml->FromUserName, $request_xml->ToUserName, $content);
                }else{
                    //$url = Yii::$app->urlManager->createAbsoluteUrl(['member/poster']);
                    //$content = '您的推广链接，<a href="'.$url.'">【请点击进入】</a>';
                    //$this->_msgText($request_xml->FromUserName, $request_xml->ToUserName, $content);
                ...
                $content = '您还未注册会员，<a href="'.$url.'">【请点击这里注册】</a>';
                $this->_msgText($request_xml->FromUserName, $request_xml->ToUserName, $content);
            }
        }
        elseif ($request_xml->EventKey == 'KEFU') {
            /*$kefuQrcode = PublicController::getSysInfo(27);
            $media = $this->uplodeTmp($kefuQrcode, 'image');
            $this->_msgImage($request_xml->FromUserName, $request_xml->ToUserName, $media);*/
    ...
    private function _doSubscribe($request_xml){
        //处理该关注事件，向用户发送关注信息
        $content = '欢迎来到'.PublicController::getSysInfo(25).'！';
        $this->_msgText($request_xml->FromUserName, $request_xml->ToUserName, $content);
    }
    /******************用户输入消息********************/
    ...
    private function _msgImage($to,$from,$file,$is_id=false){
        ...
        if($is_id){
            ...
            $result_obj = $this->uplodeTmp($file, 'image');
    ...
    public function getAccessToken($token_file = './access_token.txt'){
        ...
        //向该地址发送get请求
        $result = $this->_request('get',$url);
        // file_put_contents('result.txt',$result_obj->access_token);
        //处理响应结果
        if(!$result){
            return false;
        }
    ...
    /**
     * $type 为2的时候不需要上传到微信服务器
     * @param string $poster_path       海报路径
     * @param string $qrcode_path       生成的目录
     * @return mixed
     */
    public function createImg($openid='',$type=1,$poster_path='',$qrcode_path="./qrcode_temp/"){
        error_reporting(0);
        ...
        $font = $poster = Yii::$app->basePath.'/web/fonts/SourceHanSansCN-Medium.otf';
        $qrcode=@file_get_contents($qrcode_path.$openid.$str.".jpg");
        if(!$qrcode){
            $head_url = substr($user_info->pic,0,strripos($user_info->pic, "/"))."/46";
            //header('Content-type: image/jpg');
            //$qr_code = $this->getImg($url);
            $code_url = Yii::$app->urlManager->createAbsoluteUrl(['list/buy-agent','invitation'=>$user_info->invitation,'sign'=>202]);
            $filename_code = './uploads/img/qrcode/'.$openid.'_code.jpg';
            PublicController::qrCode($code_url,$filename_code);
            $qr_code = file_get_contents($filename_code);
            ob_end_clean();
     ...
     * @return mixed
     */
    public function createImg1($tel='',$type=1,$poster_path='',$qrcode_path="./qrcode_temp/"){
        // error_reporting(0);
        is_dir($qrcode_path) || mkdir($qrcode_path,0755,true);
        $str = substr(PublicController::getSysInfo(9),14,12);
        $poster_path = $poster_path?:ROOT.PublicController::getSysInfo(9);
        //$poster_path =$poster_path?:ROOT.'/mobile/web/images/poster.png';
        $user_info = AdminMember::find()->where(['tel'=>$tel])->one();
        $nickname = '推荐人：'.$user_info->nickname;
        $font = $poster = Yii::$app->basePath.'/web/fonts/SourceHanSerifCN-Regular.otf';
        $src = $qrcode_path.$tel.$str.".jpg";
        $qrcode=@file_get_contents($src);
        if(!$qrcode){
//            $head_url = substr($user_info->pic,0,strripos($user_info->pic, "/"))."/46";
            $code_url2 = Yii::$app->urlManager->createAbsoluteUrl(['index/register','invitation'=>$user_info->invitation,'sign'=>201]);
            $res = \mobile\controllers\PublicController::getShortUrl($code_url2);
            if($res){
                $code_url2 = $res;
            }
            $filename_code = './uploads/img/qrcode/'.$tel.'_code.jpg';
            PublicController::qrCode3($code_url2,$filename_code);
            $qr_code = file_get_contents($filename_code);
            ob_end_clean();
//            $head_img = $this->getImg($head_url);
//            ob_end_clean();
...
//            imagecopyresampled($pic, $head, 0, 0, 0, 0, 440, 440, 440, 440);
            ...
            $dst_qr = @imagecreatefromstring(file_get_contents($poster_path));
            //加水印
//            $rqys = imagecolorallocate($dst_qr, 0, 0, 0);//黑色
           // $rqys = imagecolorallocate($dst_qr, 225, 251, 240);//白色
           // return 33;
            //写的文字用到的字体。字体最好用系统有得，否则会包charmap的错，这是黑体
            //$font = "simhei.ttf";   //写的文字用到的字体。字体最好用系统有得，否则会包charmap的错，这是黑体
            // imagettftext($dst_qr,18,0,260,800,$rqys,$font,$nickname);保留
 //           imagettftext($dst_qr,18,0,226,650,$rqys,$font,$nickname); //数字参数1 字体的大小；  数字参数2  字体水平 ；数字参数3：数字越小往左
            // imagettftext($dst_qr,18,0,260,1080,$rqys,$font,'长按二维码加入');
            // imagecopy($dst_qr,$thumb, 100, 840, 0, 0, 200, 200); 
            imagecopy($dst_qr,$thumb, 228,700, 0, 0, 200, 200); //参数1 数字越大水平往右移   参数2 数字越大二维码越向下
           // imagecopy($dst_qr,$pic, 175, 770, 0, 0, 46, 46); // 用户头像
            //销毁
            //imagedestroy($thumb);
            //imagedestroy($head_img);
            ob_start();//启用输出缓存，暂时将要输出的内容缓存起来
            imagejpeg($dst_qr,$qrcode_path.$tel.$str.".jpg",90);//输出
            imagedestroy($dst_qr);
            ob_end_clean();//清空缓存

            $post_data['media'] = $qrcode_path.$tel.$str.".jpg";
            $result = $this->uplodeTmp( $post_data['media'],'image');
            if($result) {
                $media_id=$result->media_id;
            }
     ...
     * @return mixed
     */
    public function createImg2($tel='',$type=1,$poster_path='',$qrcode_path="./qrcode_temp2/"){
        ob_end_clean();//清空缓存
        is_dir($qrcode_path) || mkdir($qrcode_path,0755,true);
        $str = substr(PublicController::getSysInfo(11),14,12);
        $poster_path = $poster_path?:ROOT.PublicController::getSysInfo(11);
        //$poster_path =$poster_path?:ROOT.'/mobile/web/images/poster.png';
        $user_info = AdminMember::find()->where(['tel'=>$tel])->one();
        $nickname = '推荐人:'.$user_info->nickname;
        $font = $poster = Yii::$app->basePath.'/web/fonts/SourceHanSerifCN-Regular.otf';
        $qrcode=@file_get_contents($qrcode_path.$tel.$str.".jpg");
        if(!$qrcode){
            $head_url = substr($user_info->pic,0,strripos($user_info->pic, "/"))."/46";
            $user_id = Yii::$app->session->get('user_id');
            $code_url2 = Yii::$app->urlManager->createAbsoluteUrl(['share/loan-supermarket','uid'=>$user_id]);
            $dir = './uploads/img/qrcode/';
            is_dir($dir) || mkdir($dir,0755,true);
            $filename_code = $dir.$tel.'_code2.jpg';
            PublicController::qrCode($code_url2,$filename_code);
            $qr_code = file_get_contents($filename_code);
            ob_end_clean();
            $head_img = $this->getImg($head_url);
            ob_end_clean();
           ...
           // $pic = imagecreatetruecolor(46, 46);
            ...
            // $head = imagecreatefromstring($head_img);
            //将源文件剪切全部域并缩小放到目标图片上，前两个为资源句柄
            imagecopyresampled($thumb, $source, 0, 0, 0, 0, 250, 250, 500, 500);
          // imagecopyresampled($pic, $head, 0, 0, 0, 0, 440, 440, 440, 440);
            //创建图片的实例，接收参数为图片
            $dst_qr = @imagecreatefromstring(file_get_contents($poster_path));
            //加水印
           ...
           // return 33;
            //写的文字用到的字体。字体最好用系统有得，否则会包charmap的错，这是黑体
            //$font = "simhei.ttf";   //写的文字用到的字体。字体最好用系统有得，否则会包charmap的错，这是黑体
           // imagettftext($dst_qr,25,0,245,430,$rqys,$font,$nickname);//数字参数1 字体的大小；  数字参数2  字体水平 ；数字参数3：数字越小往左
            //imagettftext($dst_qr,18,0,260,1080,$rqys,$font,'长按二维码加入');
            imagecopy($dst_qr,$thumb, 190,420, 0, 0, 200, 200);//参数1 数字越大水平往右移   参数2 数字越大二维码越向下   图片
             // imagecopy($dst_qr,$pic, 300, 570, 0, 0, 46, 46);
            //销毁
            //imagedestroy($thumb);
            //imagedestroy($head_img);
            ob_start();//启用输出缓存，暂时将要输出的内容缓存起来
