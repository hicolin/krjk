...
class AdminUserController extends BaseController
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($model->validate() == true && $model->save()){
                $model2=new AdminUserRole();
                $model2->user_id=$model->id;
                $model2->role_id=Yii::$app->request->post('role');
                $model2->create_user = Yii::$app->user->identity->uname;
                $model2->create_date = date('Y-m-d H:i:s');
                $model2->update_user = Yii::$app->user->identity->uname;
                $model2->update_date = date('Y-m-d H:i:s');
    ...
    public function actionUpdate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($model->validate() == true && $model->save()){
                $model2=AdminUserRole::find()->where(array('user_id'=>$id))->one();
                $model2->role_id=Yii::$app->request->post('role');
                $model2->update_user = Yii::$app->user->identity->uname;
                $model2->update_date = date('Y-m-d H:i:s');
                if($model2->save(false)){
