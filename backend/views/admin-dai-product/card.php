...
<section class="content">
                ...
                <div class="box-header">
                    <div class="box-tools">
                        <div class="input-group input-group-sm" style="width: 150px;">
                            <a id="create_btn" href="<?= Url::toRoute([$this->context->id . '/create-card']) ?>"
                                ...
                                <table id="data_table" class="table table-bordered table-striped dataTable" role="grid"
                                    ...
                                    <tr role="row">
                                        ...
                                        <th tabindex="0" aria-controls="data_table" rowspan="1" colspan="1"
                                         ...
                                         <th tabindex="0" aria-controls="data_table" rowspan="1" colspan="1"
                                            aria-sort="ascending">一键推广banner显示/隐藏<br><span style="font-size: 12px;color: red;">(*建议只开启一张)</span></th>                                                                                                                                   
                                        <th tabindex="0" aria-controls="data_table" rowspan="1" colspan="1"
                                        ...
                                        <input type="text" name="listorder"  class="listorder<?=$list->id?>" onfocus="orderfocus('<?=$list->id?>')" onblur="listorder('<?=$list->id?>')" value="<?=$list->listorder?>"  style="width: 50px;height: 50px;text-indent: 20px;">
                                         ...
                                         <input type="hidden" class="focusval<?=$list->id?>">
                                           ...
                                           <td>
                                            ...
                                            <td> 
                                                   <?php
                                            if($list->is_tuibanner==0){
                                                echo "<a class='btn btn-primary btn-sm' href=".Url::toRoute([$this->context->id.'/banner','id'=>$list->id,'type'=>$list->type])." >
                                                <span class='glyphicon glyphicon-remove'>隐藏</span></a>";
                                            }elseif($list->is_tuibanner==1){
                                                echo "<a class='btn btn-danger btn-sm' href=".Url::toRoute([$this->context->id.'/banner','id'=>$list->id,'type'=>$list->type])." > <span class='glyphicon glyphicon-ok'>显示</span></a>";
                                            }
                                            ?>                    
                                            </td>
                                            <td class="center">
                                                <a id="edit_btn" class="btn btn-primary btn-sm"
                                                   href="<?= Url::toRoute([$this->context->id . '/update-card', 'id' => $list->id]) ?>">
    ...
    function listorder(id){
        ...
        $.ajax({
            type:"post",  
            url:"<?=Url::toRoute([$this->context->id . '/changeorders'])?>",
